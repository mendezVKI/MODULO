...
folder_out_name = "RESULTS_ex_04"
path = './'
folder_out_path = os.path.join(path, folder_out_name)
...
pl = pv.Plotter()
pl.add_mesh(gridData, scalars='Urandom',lighting=False, cmap=cmap_reversed,scalar_bar_args={'title': 'Velocity magnitude (m/s)'}, show_edges=False,copy_mesh=True) #,edge_opacity=0)
...
if N_PARTITIONS==1:
    ...
    # Prepare 10 partitions, see ex_05 for more details
    D = ReadData._data_processing(database=D,N_PARTITIONS=N_PARTITIONS,FOLDER_OUT='./MODULO_tmp')
    # Make sure to give the dimensions of D as input to MODULO when D is no more saved in the RAM
    m = MODULO(data=None, n_Modes=5, dtype='float64' ,weights =weights_for_D,N_PARTITIONS=N_PARTITIONS,N_S=N_P*N,N_T=N_T)
    Phi_POD, Psi_POD, Sigma_POD = m.compute_POD_K()
