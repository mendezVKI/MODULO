...
from .core._dft import dft_fit
from .core._dmd_s import dmd_s
from .core._k_matrix import CorrelationMatrix
from .core._mpod_space import spatial_basis_mPOD
from .core._mpod_time import temporal_basis_mPOD
from .core._pod_space import Spatial_basis_POD
from .core._pod_time import Temporal_basis_POD
from .core._spod_s import compute_SPOD_s
from .core._spod_t import compute_SPOD_t
from .utils._data_matrix import DataMatrix
...
class MODULO:
    ...
    def _data_processing(self,
        ...
        if not self.is_cartesian[0]:
            # Get the grid
            xx, yy = get_grid(self.is_cartesian[1])
            # Compute weight matrix
            self.W = grid_to_weights(xx, yy)
            # Compute the weighted data matrix
            self.D = self.W @ self.D
    ...
    def compute_SPOD_t(self, F_S, L_B=500, O_B=250, n_Modes=10, SAVE_SPOD=True):
        ...
        if self.D is None:
            D = np.load(self.FOLDER_OUT + '/MODULO_tmp/data_matrix/database.npz')['D']
            Phi_SP, Sigma_SP, Freqs_Pos = compute_SPOD_t(D, self.N_S,
                                                     self.N_T, F_S, L_B=L_B, O_B=O_B,
                                                     n_Modes=n_Modes, SAVE_SPOD=SAVE_SPOD,
                                                     FOLDER_OUT=self.FOLDER_OUT)
        else:
            Phi_SP, Sigma_SP, Freqs_Pos = compute_SPOD_t(self.D, self.N_S,
                                                     self.N_T, F_S, L_B=L_B, O_B=O_B,
                                                     n_Modes=n_Modes, SAVE_SPOD=SAVE_SPOD,
                                                     FOLDER_OUT=self.FOLDER_OUT)
        #
        ...
        #     # Go for the SVD
        #     U, Sigma, VT = randomized_svd(D_hat_f, n_components=n_Modes, n_iter=5,
        #                                   random_state=None)
        #
        #     Phi_SP[:, :, j] = U
    ...
    def compute_kPOD(self, M_DIST=[1,10],k_m=0.1, cent='y', n_Modes=10,alpha=1e-6):
        """
        This function implements a variant of the kernel PCA as described in 
        https://arxiv.org/pdf/2208.07746.pdf.
        ...
        :param alpha: float
                regularization for K_zeta        
        :param cent: string,
                if 'y', the matrix K will be centered (mean removal).
        :param n_Modes: float,
               number of modes to be computed 
       
        :return Psi_xi: np.array
                Temporal structures in the xi decomposition
        :return Sigma_xi: np.array
                Sigmas in the xi decomposition
        :return Phi_xi: np.array
                Spatial structures in the xi decomposition
        ...
        # Encode
        # Z_xi=np.diag(Sigma_xi)@Psi_xi.T 
        # We compute the spatial structures as projections of the data onto the Psi_xi!
        R = Psi_xi.shape[1]
        PHI_xi_SIGMA_xi = np.dot(D, (Psi_xi))
        # Initialize the output
        PHI_xi = np.zeros((n_s, R))
