...
The term :math:`\frac{1}{\sqrt{n_t}}` is a normalization factor, which ensures that the Fourier basis is orthonormal, e.g. :math:`\|\bm{\psi}_F\|^2=1`.

Considering the DFT of the signal at one specific location :math:`\bm{x}_i`, the Fourier coefficients are computed as:
...
where :math:`\bm{\Sigma}_F` is a diagonal matrix containing the modal amplitudes :math:`\sigma_F`.
